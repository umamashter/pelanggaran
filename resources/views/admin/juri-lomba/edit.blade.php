...
<style>
    ...
    .input-group-cu .form-select {
        height: 46px; padding-left: 42px; border-radius: 10px;
        border: 1.5px solid #e2e8f0; font-size: 14px; transition: border .2s, box-shadow .2s;
    }
    .input-group-cu .form-control:focus,
    .input-group-cu .form-select:focus { border-color: #16a34a; box-shadow: 0 0 0 3px rgba(22,163,74,.12); }
    .input-group-cu .form-control.is-invalid,
    .input-group-cu .form-select.is-invalid { border-color: #dc2626; background-image: none; }
    .input-group-cu-icon {
        position: absolute; left: 14px; top: 50%; transform: translateY(-50%);
        color: #94a3b8; font-size: 15px; z-index: 4; pointer-events: none;
    }
    ...
    @media (max-width: 768px) {
        .create-juri-page { margin-top: 16px; padding: 0 12px; }
...
<div class="create-juri-page">
    ...
    <nav aria-label="breadcrumb" class="breadcrumb-cu">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">Edit Juri Lomba</li>
        </ol>
    </nav>
    ...
    <div class="card create-card">
        <div class="create-card-header">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h4 class="mb-0 fw-bold" style="color: #1e293b; font-size: 18px;">Edit Juri Lomba</h4>
                    <span style="font-size: 13px; color: #64748b;">Ubah juri untuk lomba <strong>{{ $juriLomba->lomba->nama ?? '-' }}</strong>.</span>
                </div>
            </div>
        </div>
        ...
        <div class="create-card-body">
            ...
            <form action="{{ route('juri-lomba.update', $juriLomba->id) }}" method="POST">
                @csrf @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group-cu">
                                ...
                                <select name="lomba_id" class="form-select @error('lomba_id') is-invalid @enderror">
                                    ...
                                    @foreach($lombas as $l)
                                    <option value="{{ $l->id }}" {{ old('lomba_id', $juriLomba->lomba_id)==$l->id ? 'selected' : '' }}>{{ $l->nama }}</option>
                                    @endforeach
                                </select>
                                @error('lomba_id')
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group-cu" style="display:flex;gap:8px;">
                                <div style="flex:1;">
                                    ...
                                    <select id="guru_select" class="form-select" style="height:46px;padding-left:42px;border-radius:10px;border:1.5px solid #e2e8f0;font-size:14px;width:100%;">
...
(function() {
    ...
    @foreach($gurus as $g)
    guruOptions[{{ $g->id }}] = '{{ addslashes($g->nama) }}';
    @endforeach

    @foreach($existingJuri as $gid)
    selected[{{ $gid }}] = guruOptions[{{ $gid }}] || 'Guru #{{ $gid }}';
    @endforeach

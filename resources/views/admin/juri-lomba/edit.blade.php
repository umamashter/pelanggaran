...
<style>
    ...
    .ljw-chip.is-static { background: var(--lw-bg); color: var(--lw-text-2); border-color: var(--lw-border); }
    .ljw-chip.is-static .ljw-rm { background: rgba(148,163,184,.2); cursor: default; }
    ...
    .ljw-lock-note { display: flex; align-items: center; gap: 7px; font-size: 12px; font-weight: 600; color: var(--lw-text-3); }
...
<div class="lw-card lw-card-pad lw-form-card ljw-wizard">
    ...
    <div class="lw-hero" style="margin:-26px -26px 22px;border-radius:0;">
        <div class="lw-hero-grid">
            <div class="lw-hero-left">
                ...
                <div>
                    <h1 class="lw-hero-title">Edit Assignment Juri</h1>
                    <p class="lw-hero-sub">Perbarui susunan juri untuk <strong style="color:#fff;">{{ $juriLomba->lomba->nama ?? '-' }}</strong></p>
                </div>
            </div>
    ...
    @if($isLocked)
        <div class="lw-lock-banner">
            ...
            <div><b>Haflah Telah Selesai</b> — Assignment ini terkunci dan tidak dapat diubah. Data bersifat read-only.</div>
        </div>
    @endif
    ...
    <form id="ljwForm" action="{{ route('juri-lomba.update', $juriLomba->id) }}" method="POST" novalidate>
        @csrf @method('PUT')

        {{-- STEP 1 : PILIH LOMBA --}}
        <section class="lw-wizard-pane is-show" data-pane="1">
            <div class="lw-form-section"><i class="bi bi-flag-fill"></i> Pilih Kompetisi</div>
            <p class="lw-help-text" style="margin:-10px 0 14px;">Klik kartu untuk mengganti lomba assignment ini.</p>

            <div class="lw-search" style="margin-bottom:14px;">
                <i class="bi bi-search"></i>
                <input type="search" class="lw-control" id="ljwLombaSearch" placeholder="Cari nama lomba..." autocomplete="off" @if($isLocked) disabled @endif>
            </div>

            @forelse($lombas as $lomba)
                <div class="lw-pick-grid" id="ljwLombaGrid" style="margin-bottom:14px;">
                    <button type="button" class="lw-pick-card {{ $lomba->id == $juriLomba->lomba_id ? 'is-picked' : '' }}"
                        data-id="{{ $lomba->id }}" data-nama="{{ e($lomba->nama) }}" data-jenis="{{ $lomba->jenis }}" @if($isLocked) disabled @endif>
            ...
            <div class="lw-wizard-nav">
                ...
                <button type="button" class="lw-btn lw-btn--solid" id="ljwNextBtn" @if($isLocked) disabled @endif>Lanjut ke Juri <i class="bi bi-arrow-right"></i></button>
            </div>
        </section>
        ...
        <section class="lw-wizard-pane" data-pane="2">
            ...
            <div class="lw-search" style="margin-bottom:14px;">
                <i class="bi bi-search"></i>
                <input type="search" class="lw-control" id="ljwGuruSearch" placeholder="Cari guru berdasarkan nama atau NIP..." autocomplete="off" @if($isLocked) disabled @endif>
            </div>

            @forelse($gurus as $guru)
                <div class="ljw-guru-grid" id="ljwGuruGrid" style="margin-bottom:14px;">
                    <button type="button" class="ljw-guru-card {{ in_array($guru->id, $existingJuri) ? 'is-picked' : '' }}"
                        data-id="{{ $guru->id }}" data-nama="{{ e($guru->nama) }}" data-nip="{{ $guru->nip ?? '' }}" @if($isLocked) disabled @endif>
            ...
            <div class="ljw-selected is-show" id="ljwSelected">
                <div class="ljw-selected-title"><i class="bi bi-check-circle-fill"></i> Juri Saat Ini <span class="ljw-count-badge" id="ljwSelCount">{{ count($existingJuri) }}</span></div>
                <div class="ljw-chips" id="ljwChipWrap"></div>
            </div>

            <div class="ljw-preview is-show" id="ljwPreview">
                ...
                <div class="ljw-preview-body">
                    <div class="ljw-preview-stats">
                        <div class="ljw-preview-stat"><div class="val" id="ljwLiveTotal">{{ count($existingJuri) }}</div><div class="lbl">Total Juri</div></div>
                        <div class="ljw-preview-stat"><div class="val" id="ljwLiveNama">{{ $juriLomba->lomba->nama ?? '-' }}</div><div class="lbl">Lomba</div></div>
                    </div>
            ...
            <div class="lw-wizard-nav">
                @if($isLocked)
                    <span class="ljw-lock-note"><i class="bi bi-lock-fill"></i> Data terkunci — tidak dapat diubah</span>
                @else
                ...
                <button type="submit" class="lw-btn lw-btn--solid" id="ljwSubmitBtn" @if($isLocked) disabled @endif>
                    <span class="ljw-btn-label"><i class="bi bi-save-fill"></i> Simpan Perubahan</span>
...
(function () {
    ...
    /* ---------- prefill guru selection ---------- */
    @foreach($existingJuri as $gid)
        @php $g = $gurus->firstWhere('id', $gid); @endphp
        @if($g) selectedGurus[{{ $gid }}] = @json($g->nama); @endif
    @endforeach
    var currentLombaId = {{ $juriLomba->lomba_id }};
    ...
    function syncLombaInput() { lombaInput.value = selectedLomba ? selectedLomba.id : currentLombaId; }
    ...
    function renderChips() {
        ...
        var readyEl = $id('ljwReady');
        if (readyEl) { readyEl.classList.toggle('is-show', cnt > 0 && !!selectedLomba); }
        $id('ljwSubmitBtn').disabled = isLocked || cnt < 1;
        ...
        ids.forEach(function (id) {
            ...
            chip.className = 'ljw-chip' + (isLocked ? ' is-static' : '');
            chip.innerHTML = '<i class="bi bi-bank"></i>' + selectedGurus[id] + (isLocked ? '' : '<span class="ljw-rm" data-rm="' + id + '" aria-label="Hapus">&times;</span>');
    ...
    /* ── init ── */
    var initCard = document.querySelector('.lw-pick-card.is-picked');
    if (initCard) {
        selectedLomba = { id: initCard.dataset.id, nama: initCard.dataset.nama, jenis: initCard.dataset.jenis };
    }
    syncLombaInput();
    renderChips();
})();

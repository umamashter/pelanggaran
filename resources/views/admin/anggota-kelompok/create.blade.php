...
<div class="lw-card lw-card-pad lw-form-card">
    ...
    <div class="lw-hero" style="margin:-26px -26px 22px;border-radius:0;">
        <div class="lw-hero-grid">
            <div class="lw-hero-left">
                ...
                <div>
                    <h1 class="lw-hero-title">Tambah Anggota Kelompok</h1>
                    <p class="lw-hero-sub">Wizard keanggotaan — pilih kelompok tim, lalu pilih siswa yang akan didaftarkan sebagai anggota (minimal 2).</p>
                </div>
            </div>
    ...
    <form action="{{ route('anggota-kelompok.store') }}" method="POST" id="formAnggota" novalidate>
        @csrf

        {{-- STEP 1 : KELOMPOK --}}
        <section class="lw-wizard-pane is-show" data-pane="1">
            <div class="lw-form-section"><i class="bi bi-people-fill"></i> Pilih Kelompok</div>
            <p class="lw-help-text" style="margin:-10px 0 14px;">Pilih kelompok yang belum memiliki anggota.</p>
            ...
            @if($kelompokLombas->isEmpty())
                <div class="lw-empty">
                    ...
                    <div class="lw-empty-title">Semua Kelompok Sudah Lengkap</div>
                    <p class="lw-empty-sub">Tidak ada kelompok tanpa anggota. Buat kelompok baru terlebih dahulu.</p>
            ...
            <div class="lw-pick-grid" id="kelompokGrid">
                @foreach($kelompokLombas as $kl)
                    ...
                    @endphp
                    <button type="button" class="lw-pick-card" data-id="{{ $kl->id }}"
                        data-kelas-min="{{ $l->kelas_min ?? '' }}" data-kelas-max="{{ $l->kelas_max ?? '' }}"
                        data-nama="{{ $kl->nama_kelompok }}" data-lomba="{{ $l->nama ?? '-' }}" {{ old('kelompok_lomba_id') == $kl->id ? 'data-old' : '' }}>
        ...
        <section class="lw-wizard-pane" data-pane="2">
            ...
            <input type="hidden" name="kelompok_lomba_id" id="kelompok_lomba_id" value="{{ old('kelompok_lomba_id') }}">
            ...
            <div class="lw-pick-panel" id="siswaPanel">
                ...
                <div id="siswaList">
                    <div class="lw-empty" style="padding:28px 16px;">
                        <div class="lw-empty-title" style="font-size:14px;">Belum Ada Kelompok Dipilih</div>
                        <p class="lw-empty-sub" style="font-size:12px;">Pilih kelompok terlebih dahulu pada langkah 1.</p>
                    </div>
                </div>
        ...
        <section class="lw-wizard-pane" data-pane="3">
            ...
            <div class="lw-wizard-nav">
                ...
                <button type="submit" class="lw-btn lw-btn--solid" data-submit-button>
                    <span class="btn-label"><i class="bi bi-save-fill"></i> Simpan Anggota</span>
...
(function () {
    ...
    function loadStudents(kelompokId) {
        ...
        fetchJSON('{{ url('anggota-kelompok/get-siswa') }}/' + kelompokId, function (data) {
            ...
            studentRows = (data.students || []).map(function (item) {
                ...
                var nisn = m ? m[2] : '';
                return { id: String(item.id), name: name, nisn: nisn, text: item.text,
    ...
    if (hasOld) {
        var oldCard = document.querySelector('#kelompokGrid .lw-pick-card[data-old]');
        if (oldCard) {
            oldCard.click();
            setTimeout(function () {
                goStep(2);
                loadStudents(state.id);
                setTimeout(function () {
                    var olds = @json(old('student_ids', []));
                    studentRows.forEach(function (s) {
                        if (olds.indexOf(Number(s.id)) !== -1 || olds.indexOf(s.id) !== -1) { picked[s.id] = s.text; }
                    });
                    renderList();
                }, 450);
            }, 300);

...
<style>
    ...
    .select2-multi-cu.is-invalid + .select2-container .select2-selection--multiple {
        border-color: #dc2626;
    }
...
<div class="create-anggota-page">
    ...
    <nav aria-label="breadcrumb" class="breadcrumb-cu">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">Tambah Anggota Kelompok</li>
        </ol>
    </nav>
    ...
    <div class="card create-card">
        <div class="create-card-header">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h4 class="mb-0 fw-bold" style="color: #1e293b; font-size: 18px;">Tambah Anggota Kelompok</h4>
                    <span style="font-size: 13px; color: #64748b;">Pilih kelompok dan tambahkan beberapa siswa sekaligus (minimal 2 siswa).</span>
                </div>
            </div>
        </div>
        ...
        <div class="create-card-body">
            ...
            <form action="{{ route('anggota-kelompok.store') }}" method="POST" id="formAnggota">
                @csrf
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group-cu">
                                ...
                                <select name="kelompok_lomba_id" id="kelompok_lomba_id" class="form-select @error('kelompok_lomba_id') is-invalid @enderror">
                                    ...
                                    <option value="{{ $kl->id }}" data-kelas-min="{{ $kl->lomba->kelas_min ?? '' }}" data-kelas-max="{{ $kl->lomba->kelas_max ?? '' }}" {{ old('kelompok_lomba_id')==$kl->id ? 'selected' : '' }}>{{ $kl->nama_kelompok }}</option>
                                    @endforeach
                                </select>
...
$(document).ready(function () {
    ...
    var oldStudentIds = @json(old('student_ids', []));
    var oldKelompokId = @json(old('kelompok_lomba_id'));

    $siswa.select2({
        placeholder: 'Pilih kelompok terlebih dahulu...',
        width: '100%'
    });
    ...
    if (oldKelompokId) {
        $kelompok.val(oldKelompokId).trigger('change');
    } else {
        resetStudents('Pilih kelompok terlebih dahulu...');
    }
});

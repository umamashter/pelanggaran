...
<style>
    ...
    .edit-card { border: none; border-radius: 18px; box-shadow: 0 4px 16px rgba(0,0,0,.06), 0 2px 8px rgba(0,0,0,.04); }
    .edit-card-header { padding: 24px 28px 20px; border-bottom: 1px solid #f1f5f9; }
    .edit-card-body { padding: 24px 28px 28px; }
    ...
    .btn-cu-outline { background: transparent; color: #16a34a; border: 1.5px solid #16a34a; }
    .btn-cu-outline:hover { background: #f0fdf4; transform: translateY(-1px); }
    .btn-cu-danger { background: transparent; color: #ef4444; border: 1.5px solid #fca5a5; width: 38px; height: 38px; padding: 0; border-radius: 10px; display: inline-flex; align-items: center; justify-content: center; flex-shrink: 0; transition: all .2s; }
    .btn-cu-danger:hover { background: #fef2f2; border-color: #ef4444; }
    ...
    .aspek-item { display: flex; align-items: center; gap: 12px; padding: 12px 16px; background: #f8fafc; border-radius: 12px; border: 1.5px solid #e2e8f0; transition: all .2s; }
    .aspek-item:hover { border-color: #cbd5e1; background: #f1f5f9; }
    .aspek-number { width: 30px; height: 30px; border-radius: 8px; background: #e2e8f0; color: #475569; font-size: 13px; font-weight: 700; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
    .aspek-item .form-control { border: 1.5px solid #e2e8f0; border-radius: 10px; height: 42px; font-size: 14px; padding: 0 14px; transition: all .2s; }
    .aspek-item .form-control:focus { border-color: #22c55e; box-shadow: 0 0 0 3px rgba(34,197,94,.12); }
    .aspek-item .form-control::placeholder { color: #94a3b8; }
    #addRowBtn { height: 38px; padding: 0 16px; border-radius: 10px; font-size: 13px; font-weight: 600; display: inline-flex; align-items: center; gap: 6px; }
    @media (max-width: 768px) {
        .edit-aspek-page { margin-top: 16px; padding: 0 12px; }
        .edit-card-header, .edit-card-body { padding: 18px 20px; }
        .form-actions-cu { flex-direction: column; }
        .form-actions-cu .btn-cu { width: 100%; }
        .aspek-item { padding: 10px 12px; }
    }
</style>
@endpush
@section('content')
@include('component.admin.ms-style')
<div class="edit-aspek-page">
    ...
    <div class="card edit-card">
        <div class="edit-card-header">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h4 class="mb-0 fw-bold" style="color: #1e293b; font-size: 18px;">Edit Aspek Penilaian</h4>
                    <span style="font-size: 13px; color: #64748b;">Ubah aspek penilaian untuk lomba <strong>{{ $lomba->nama }}</strong></span>
                </div>
            </div>
        </div>
        ...
        <div class="edit-card-body">
            ...
            @if ($errors->any())
            <div class="alert" style="border:none;border-radius:12px;padding:14px 20px;font-size:14px;background:#fef2f2;color:#991b1b;border-left:4px solid #dc2626;margin-bottom:20px;">
            ...
            <form action="{{ route('aspek-penilaian.update', $aspekPenilaians->first()->id) }}" method="POST">
                @csrf @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <select name="lomba_id" class="form-control @error('lomba_id') is-invalid @enderror" style="border:1.5px solid #e2e8f0;border-radius:10px;height:44px;font-size:14px;" required>
                        ...
                        @foreach($lombas as $l)
                        <option value="{{ $l->id }}" {{ old('lomba_id', $lomba->id) == $l->id ? 'selected' : '' }}>{{ $l->nama }}</option>
                        @endforeach
                    </select>
                    @error('lomba_id')
                ...
                <div id="aspekRows" style="display:flex;flex-direction:column;gap:10px;">
                    @php $existing = old('nama_aspek') ?: $aspekPenilaians->pluck('nama_aspek')->toArray(); @endphp
                    @forelse($existing as $idx => $nama)
                    <div class="aspek-item">
                        <span class="aspek-number">{{ $idx + 1 }}</span>
                        <input type="text" name="nama_aspek[]" class="form-control" placeholder="Nama aspek penilaian" value="{{ $nama }}" required style="flex:1;">
                        <button type="button" class="btn-cu-danger removeRow" title="Hapus"><i class="fas fa-times"></i></button>
                    </div>
                    @empty
                    <div class="aspek-item">
                        <span class="aspek-number">1</span>
                        <input type="text" name="nama_aspek[]" class="form-control" placeholder="Nama aspek penilaian" value="" required style="flex:1;">
                        <button type="button" class="btn-cu-danger removeRow" title="Hapus"><i class="fas fa-times"></i></button>
                    </div>
                    @endforelse
                </div>
                ...
                <div class="form-actions-cu">
                    <a href="{{ route('aspek-penilaian.show', $lomba->id) }}" class="btn btn-cu btn-cu-secondary">

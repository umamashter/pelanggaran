...
.btn-pdf-ms { padding: 8px 20px; border-radius: 10px; font-size: 13px; font-weight: 600; border: none; background: linear-gradient(135deg, #dc2626, #ef4444); color: #fff; transition: all .25s; display: inline-flex; align-items: center; gap: 6px; box-shadow: 0 2px 8px rgba(220,38,38,.25); text-decoration: none; height: 40px; }
.btn-pdf-ms:hover { transform: translateY(-1px); box-shadow: 0 4px 14px rgba(220,38,38,.35); color: #fff; }
.btn-kembali-ms { padding: 10px 20px; border-radius: 10px; font-size: 13px; font-weight: 500; border: 1.5px solid var(--ms-border); background: #fff; color: #475569; transition: all .25s; display: inline-flex; align-items: center; gap: 6px; text-decoration: none; }
...
.table-rekap { border-collapse: collapse; width: 100% !important; border: 1px solid var(--ms-border); border-radius: 12px; margin: 0 !important; }
.table-rekap thead th { background: #f8fafc; color: #475569; font-weight: 600; font-size: 12px; text-transform: uppercase; letter-spacing: .4px; padding: 11px 14px; border-bottom: 2px solid var(--ms-border); white-space: nowrap; text-align: center; }
.table-rekap tbody td { padding: 10px 14px; font-size: 13px; color: #334155; border-bottom: 1px solid #f1f5f9; vertical-align: middle; line-height: 1.5; }
.table-rekap tbody tr:last-child td { border-bottom: none; }
.table-rekap tbody tr:hover td { background: #f8fafc; }
.badge-count { display: inline-block; padding: 3px 10px; border-radius: 12px; font-size: 12px; font-weight: 600; min-width: 28px; text-align: center; }
...
@media (max-width: 768px) { .table-card .card-body { padding: 12px 14px 16px; } .table-rekap thead th { font-size: 11px; padding: 9px 8px; } .table-rekap tbody td { padding: 8px; font-size: 12px; } }
</style>
        ...
        <div class="card-body p-4">
            <div class="d-flex flex-column flex-xl-row justify-content-between align-items-xl-center gap-3">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <h4 class="mb-1 fw-bold" style="color: var(--ms-text); font-size: 20px;">Rekap Absensi Siswa</h4>
        ...
        <div class="card-body">
            <form method="GET" class="row g-3 align-items-end">
                ...
                <div class="col-md-2">
                    ...
                    <input type="month" name="bulan" value="{{ request('bulan', $bulan ?? '') }}" class="form-control">
                </div>
                <div class="col-md-2">
                    <label class="form-label"><i class="fas fa-calendar me-1" style="color:var(--ms-primary);"></i>Dari Tanggal</label>
                    <input type="date" name="tanggal_awal" value="{{ request('tanggal_awal', $tanggalAwal ?? '') }}" class="form-control">
                </div>
                <div class="col-md-2">
                    <label class="form-label"><i class="fas fa-calendar me-1" style="color:var(--ms-primary);"></i>Sampai Tanggal</label>
                    <input type="date" name="tanggal_akhir" value="{{ request('tanggal_akhir', $tanggalAkhir ?? '') }}" class="form-control">
                </div>
                <div class="col-md-3 d-flex gap-2">
                    <button class="btn-filter-ms"><i class="fas fa-search"></i> Filter</button>
                    @if(request('kelas_id'))
                    <a href="{{ route('absensi.rekap.pdf', ['kelas_id' => request('kelas_id'), 'bulan' => request('bulan'), 'tanggal_awal' => request('tanggal_awal'), 'tanggal_akhir' => request('tanggal_akhir')]) }}" target="_blank" class="btn-pdf-ms">
                ...
                <table class="table table-rekap">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center"><span class="badge-count hadir">{{ $data['hadir'] }}</span></td>
                            <td class="text-center"><span class="badge-count izin">{{ $data['izin'] }}</span></td>
                            <td class="text-center"><span class="badge-count sakit">{{ $data['sakit'] }}</span></td>
                            <td class="text-center"><span class="badge-count alpha">{{ $data['alpa'] }}</span></td>
                    ...
                    <tfoot>
                        <tr style="background:#f8fafc;">
                            ...
                            <th class="text-center">{{ $totalH }}</th>
                            <th class="text-center">{{ $totalI }}</th>
                            <th class="text-center">{{ $totalS }}</th>
                            <th class="text-center">{{ $totalA }}</th>
                            <th class="text-center">{{ $totalH + $totalI + $totalS + $totalA }}</th>
                        </tr>
                    </tfoot>
                </table>
            </div>

...
@section('title', 'Data Guru (Read Only)')
@push('css')
...
<style>
    ...
    @media (max-width: 575.98px) { .action-group-ms { display: inline-flex !important; gap: 4px !important; grid-template-columns: unset !important; } }
</style>
@endpush
    ...
    <div class="card border-0 shadow-sm mb-4" style="border-radius: 16px;">
        <div class="card-body p-4 d-flex flex-column flex-xl-row justify-content-between align-items-xl-center gap-3">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h4 class="mb-1 fw-bold" style="color: var(--ms-text); font-size: 20px;">Data Guru</h4>
                    <span class="badge-modern badge-ta"><i class="fas fa-address-book me-1"></i>{{ $gurus->count() }} Guru</span>
                    @if(isset($jenjang))
        ...
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
                <div style="font-size:16px;font-weight:700;color:var(--ms-text,#1e293b);display:flex;align-items:center;gap:10px;">
                    <i class="fas fa-table" style="color:var(--ms-primary,#16a34a);font-size:18px;"></i> Daftar Guru
                </div>
                <div class="ms-search-box">
                    <i class="fas fa-search ms-search-icon"></i>
                    <input type="text" id="searchInput" class="ms-search-input" placeholder="Cari nama atau kode guru...">
                </div>
            </div>
            ...
            <table id="guruTable" class="table table-ms display" cellspacing="0" width="100%">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><code>{{ $guru->kode_guru }}</code></td>
                        <td>{{ $guru->nama }}</td>
                        <td>{{ $guru->nip ?? '-' }}</td>
                        <td>{{ $guru->no_hp ?? '-' }}</td>
                    </tr>
